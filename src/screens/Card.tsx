    ...
    <Card sx={{ maxWidth: 500, borderRadius: 5 }}>
      <CardActionArea >
        ...
        <CardContent >
          ...
          <LocationOnIcon sx={{color: '#748867', marginRight: 1  }} fontSize='small' />
          <Typography variant="body2" >
            {item.location}
          </Typography>
          </Box>
          <Box sx={{flexDirection:'row', display:'flex'}} my={1}>
          <GroupsIcon sx={{color: '#748867', alignSelf:'center', marginRight:1 }}  fontSize='small' />
          <Typography variant="body2" >
            {item.people[0]} - {item.people[item.people.length-1]} persona
          </Typography>
          </Box>
          <Box sx={{flexDirection:'row', display:'flex'}}>
          <BedroomParentIcon sx={{color: '#748867', alignSelf:'center', marginRight: 1 }} fontSize='small' />
          <Typography variant="body2" >
        {item.rooms[item.rooms.length-1]} {item.rooms.length === 1 && item.rooms[0] === "1"  ? "dhomë" : "dhoma"}
          </Typography>
          </Box>
        </CardContent>
      </CardActionArea>
